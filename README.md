...
Αν λοιπόν ο επεξεργαστής με ισχύ 40 W εκτελεί τον επιθυμητό αριθμό εντολών σε χρόνο μικρότερο από το ένα δέκατο του χρόνου που χρειάζεται ο επεξεργαστής των 4 W τότε ενδέχεται να μπορεί να προσφέρει στο σύστημα μεγαλύτερη διάρκεια μπαταρίας (με εξάρτηση και στον μέσο φόρτο εργασίας που δέχεται ο επεξεργαστής).
Οπότε δεν μπορούμε να αποφανθούμε με βεβαιότητα ποιος επεξεργαστής μπορεί να προσφέρει μεγαλύτερη διάρκεια μπαταρίας στο σύστημα.
Για να απαντήσουμε το ερώτημα χρειαζόμαστε επιπλέον τον χρόνο που θα χρειαστεί ο καθένας για να εκτελέσει τις επιθυμητές εντολές αλλά και την κατανομή του συνολικού power σε dynamic και leakage.
...
Στην περίπτωση που το σύστημα τερμάτιζε τη λειτουργία του μετά την εκτέλεση του προγράμματος τότε ο Xeon θα ήταν πιο energy efficient καθώς δε θα λαμβανόταν υπόψιν ο όρος _39 * 36.8319_.
...
Οι βέλτιστες επιλογές που προέκυψαν στη δεύτερη εργασία από τη συνάρτηση _Performance / Cost_ είναι αντίστοιχα:
...
Παρατηρούμε ότι η μεγιστοποίηση του performance και του energy efficiency ανά μονάδα κόστους μας οδηγούν σε διαφορετικές επιλογές.
Για την τελική επιλογή σχεδίασης ενός συστήματος θα χρειαστεί να σταθμιστούν οι βέλτιστες τιμές που προκύπτουν από τις δύο αυτές μελέτες.
...
Παρατηρούμε ότι δεν υπάρχουν σημαντικές διαφορές ανάμεσα στις τιμές peak power από benchmark σε benchmark και αυτό εξηγείται από το γεγονός ότι τουλάχιστον για μια στιγμή το κάθε benchmark παρέχει πλήρες φορτίο στον επεξεργαστή.
...
Ένας παράγοντας που λείπει από την ανάλυσή μας είναι ο τελικός σκοπός χρήσης του συστήματος, ο οποίος μπορεί να διαφοροποιήσει σημαντικά την τελική επιλογή. Χρήσιμες παράμετροι στην ανάλυση θα ήταν ο μέσος φόρτος εργασίας που δέχεται ο επεξεργαστής και η ανάγκη για βέλτιστη ενεργειακή διαχείριση.
Επιπλέον, η συμπεριφορά ενός πραγματικού συστήματος εξαρτάται σε μεγάλο βαθμό από τη θερμοκρασία του περιβάλλοντος χώρου, κάτι το οποίο δεν έχει ληφθεί υπόψιν κατά την ανάλυση μας.
Η χρήση διαφορετικών προγραμμάτων για τη μελέτη του ιδίου συστήματος μπορεί να οδηγήσει σε σφάλματα στην περίπτωση που το κάθε πρόγραμμα μοντελοποιεί διαφορετικά το σύστημα υπό μελέτη.
Τα ακριβή μοντέλα των περισσότερων συστημάτων δε δημοσιοποιούνται από τις κατασκευάστριες εταιρίες για λόγους ανταγωνισμού οπότε τα μοντέλα που χρησιμοποιούμε αποτελούν μια high-level περιγραφή του συστήματος παραλείποντας τις low-level λεπτομέρειες.
Επίσης, όταν τα αποτελέσματα του ενός προγράμματος έχουν απόκλιση από τα πραγματικά και εισάγονται ως είσοδος σε ένα δεύτερο πρόγραμμα ακόμα και μια μικρή απόκλιση μπορεί να αποδειχθεί σημαντική στο τελικό αποτέλεσμα.
Αν για παράδειγμα ο παράγοντας που παρουσιάζει την απόκλιση αποτελεί εκθετικό παράγοντα σε κάποιον υπολογισμό του δεύτερου προγράμματος το τελικό σφάλμα θα είναι πολύ μεγαλύτερο από την αρχική απόκλιση του πρώτου προγράμματος.
Αντίστοιχα, σφάλματα μπορούν να προκύψουν όταν η μοντελοποίηση του συστήματος γίνεται σε high-level γλώσσα από την πλευρά του χρήστη χωρίς να προσδιορίζονται low-level λεπτομέρειες<sup>[[3]](#πηγές)</sup>.
...
- Μελετήσαμε τις βασικές κατηγορίες ισχύος που χαρακτηρίζουν ένα ολοκληρωμένο κύκλωμα
- Εξοικειωθήκαμε με τον McPAT
- Προβληματιστήκαμε για τις παραμέτρους που χρειάζεται να ληφθούν υπόψιν κατά τη σχεδίαση διαφορετικών συστημάτων και τις σταθμίσεις που χρειάζεται να γίνουν
- Προβληματιστήκαμε για τις πιθανές πηγές σφαλμάτων που χρειάζεται να έχουμε κατά νου κατά τη διάρκεια της μελέτης/ανάλυσης ενός συστήματος

Μεγάλο ενδιαφέρον παρουσίασε η σύγκριση των αποτελεσμάτων με τα αντίστοιχα αποτελέσματα της προηγούμενης εργασίας απ' όπου φάνηκε ότι δεν υπάρχει μια προφανής βέλτιστη επιλογή για τις παραμέτρους του συστήματος και πως χρειάζεται να γίνει στάθμιση για να προκύψει η τελική επιλογή.

Η δυσκολία της εργασίας ήταν μέτρια και η καθοδήγηση από την εκφώνηση πολύ επαρκής.
