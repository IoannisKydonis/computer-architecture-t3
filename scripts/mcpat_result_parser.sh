...
declare -a parameters=("Area" "Subthreshold Leakage" "Gate Leakage" "Runtime Dynamic" "Peak Dynamic")
...
for b in "${benchmarks[@]}"; do
    ...
    echo " Total Power | Peak Power | Execution Time | Energy | Energy Efficiency | Energy Efficiency / Area |" >> "$res_file"
    echo -n "| --- | --- | --- | --- | --- | --- | --- |" >> "$res_file"
    for pm in "${parameters[@]}"; do
        echo -n " --- |" >> "$res_file"
    done
    ...
    for o in "${options[@]}"; do
        ...
        for pm in "${parameters[@]}"; do
            ...
            if [ "$pm" = "Subthreshold Leakage" ] || [ "$pm" = "Gate Leakage" ] || [ "$pm" = "Peak Dynamic" ]; then
                peak_power=$(echo "$peak_power + $val" | bc)
            fi
            echo -n " $val |" >> "$res_file"
        done
        ...
        echo -n " $total_power | $peak_power | $exec_time | $energy | $energy_efficiency | $energy_efficiency_per_area |" >> "$res_file"
        echo "" >> "$res_file"
    done
done
